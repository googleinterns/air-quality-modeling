...
class CollectionClass(object):
    ...
    def get_clipped_range_bands(self, geometry, start_date, end_date):
        return self.imagery.filterBounds(geometry
                            ).filterDate(start_date, end_date).map(
                                                lambda x: x.clip(geometry))
...
class TropomiImagery(CollectionClass):
    def __init__(self, link, bands=None, before_range=(0,'day'), after_range=(7,'day')):
...
class MultiSpectralImagery(CollectionClass):
    def __init__(self, link, start_date, end_date, bands=None):
        ...
        self.imagery = self.imagery.filter(ee.Filter.notNull(['collectionStartTime']))
...
class DSMImagery(CollectionClass):
    def get_bands(self, date, geometry, scale=None):
        images = self.imagery.filterBounds(geometry)
        
        def add_hour_difference(img):
            return img.setMulti({"hour_difference":
                                     date.difference(img.date(),'hour')})
...
class WindImagery(CollectionClass):
    ...
    def get_bands(self, date, geometry):
        ...
        images = images.filter(ee.Filter.date(date.advance(*self.before),
                                                  date))
        images = images.map(lambda img:self.stack_bands(img,date).clip(geometry))
